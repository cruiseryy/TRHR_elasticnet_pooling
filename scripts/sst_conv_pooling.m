...
omask = mask0.*ocean_mask; % ocean mask after convolutional layer
...
for i = 1:ni
    for j = 1:nj
        if sum(sum(omask((i-1)*wsz+1:i*wsz, (j-1)*wsz+1:j*wsz))) == wsz^2
            
%             rsst(i,j,:) = reshape(mean(mean(sst((i-1)*wsz+1:i*wsz, (j-1)*wsz+1:j*wsz, :))),nt,1); % average pooling
            rsst(i,j,:) = reshape(max(max(ssta((i-1)*wsz+1:i*wsz, (j-1)*wsz+1:j*wsz, :))),nt,1); % max pooling
%             pause = 1;
...
for i = 1:ni
    for j = 1:nj
        if rmask(i,j) == 1
            sstv(:,tmark) = reshape(rsst(i,j,:),nt,1);
            loc(:,tmark) = [i;j];
            tmark = tmark + 1;
        end
    end
